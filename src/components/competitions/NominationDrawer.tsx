...
export default function NominationDrawer({
  ...
  const [athleteStates, setAthleteStates] = useState<Record<string, AthleteNominationState>>({});
    ...
    if (athletesData) {
      ...
      athletesData.forEach((athlete) => {
        ...
        const existingNomination = existingNominations.find(
          (nomination) => {
            const nominationAthleteId = typeof nomination.athleteId === "string" 
        ...
        initialStates[athlete._id] = {
          ...
          weightCategory: existingNomination?.weightCategory || null,
  ...
  const handleSave = async () => {
    ...
    if (nominationsToCreate.length === 0 && nominationsToDelete.length === 0) return;

    setLoading(true);
    try {
      // Handle nominations to create
      if (nominationsToCreate.length > 0) {
        const createResponse = await batchCreateNominations(nominationsToCreate);
      ...
      setAthleteStates((prev) => {
        const updated = { ...prev };
        nominationsToCreate.forEach((nomination) => {
          updated[nomination.athleteId] = {
            ...updated[nomination.athleteId],
            status: "nominated",
          };
        });
        nominationsToDelete.forEach((nominationId) => {
          const nomination = existingNominations.find(n => n._id === nominationId);
          if (nomination) {
            const athleteId = typeof nomination.athleteId === "string"
              ? nomination.athleteId
              : nomination.athleteId._id;
            updated[athleteId] = {
              ...updated[athleteId],
      ...
      // Update nominations list
      setExistingNominations(prev => 
        prev.filter(n => !nominationsToDelete.includes(n._id))
      );
  ...
  const handleUnnominate = async (nomination: Nomination) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        setExistingNominations(prev => 
          prev.filter(n => {
            const nAthleteId = typeof n.athleteId === "string" 
              ? n.athleteId 
              : n.athleteId._id;
  ...
  const hasPendingChanges = Object.values(athleteStates).some(
    (state) => state.status === "pending" || state.status === "pendingUnnominate"
  );
  ...
  return (
    <>
      ...
      >
        <Paper pos="relative">
          <LoadingOverlay visible={loading || athletesLoading || nominationsLoading} />
          <Stack spacing="sm">
                  ...
                  <Group position="apart" align="center" noWrap>
                    <Group spacing="sm" noWrap>
                      <ActionIcon
                        ...
                        color={
                          isPending ? "yellow" :
                          isPendingUnnominate ? "red" :
                          isNominated ? "green" : "gray"
                        }
                        onClick={() => {
                          if (isNominated) {
                            handleAthleteStatusChange(athlete._id, "pendingUnnominate");
                    ...
                    <Group spacing="xs" noWrap>
                      <Select
                        ...
                        value={state?.weightCategory || null}
                        onChange={(value) =>
      ...
      <Modal
        opened={!!nominationToDelete}
        onClose={() => setNominationToDelete(null)}
        title={t("nominations.confirmUnnominate")}
        centered
      >
        <Stack>
          <Text>
            {t("nominations.confirmUnnominateMessage")}
          </Text>
          <Group position="right">
            ...
            <Button
              color="red"
              onClick={() => nominationToDelete && handleUnnominate(nominationToDelete)}
              loading={loading}
            >
              {t("nominations.unnominate")}
            </Button>
          </Group>
        </Stack>
      </Modal>
