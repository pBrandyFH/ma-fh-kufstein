...
import type { InvitationFormValues, SelectOption, UserRole } from "../../types"
...
export function InvitationForm({ onSuccess }: InvitationFormProps) {
  ...
  const getAvailableRoles = (): SelectOption[] => {
  ...
  const shouldShowFederationField = (): boolean => {
    const role = form.values.role as UserRole
    return (
  ...
  const shouldShowClubField = (): boolean => {
    const role = form.values.role as UserRole
