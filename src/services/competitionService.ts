...
export const getAllCompetitions = async (): Promise<
  ApiResponse<CompetitionWithAthleteCount[]>
> => {
  try {
    const response = await api.get<ApiResponse<CompetitionWithAthleteCount[]>>(
      "/competitions"
    );
...
export const getCompetitionsByFederation = async (
  federationId: string
): Promise<ApiResponse<Competition[]>> => {
  try {
    const response = await api.get<ApiResponse<Competition[]>>(
      `/competitions/federation/${federationId}`
    );
    return response.data;
  } catch (error) {
    if (error instanceof Error) {
...
export const getUpcomingCompetitions = async (): Promise<
...
export const getCompetitionsByClub = async (
  clubId: string
): Promise<ApiResponse<Competition[]>> => {
  try {
    const response = await api.get<ApiResponse<Competition[]>>(
...
export async function getFilteredCompetitions(): Promise<
  ApiResponse<Competition[]>
> {
  try {
    const response = await api.get<ApiResponse<Competition[]>>(
...
export async function getInternationalCompetitions(
  ...
  try {
    const response = await api.get<ApiResponse<Competition[]>>(
      `/competitions/international/${federationId}`
    );
    return response.data;
  } catch (error) {
    if (error instanceof Error) {
...
export async function getNationalCompetitions(
  federationId: string
): Promise<ApiResponse<Competition[]>> {
  try {
    const response = await api.get<ApiResponse<Competition[]>>(
      `/competitions/national/${federationId}`
    );
    return response.data;
  } catch (error) {
    if (error instanceof Error) {
...
export async function getCompetitionByHostFederationId(
  federationId: string
): Promise<ApiResponse<Competition[]>> {
  try {
    const response = await api.get<ApiResponse<Competition[]>>(
